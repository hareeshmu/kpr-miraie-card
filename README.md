...
Designed to pair with the **[KPR MirAIe Local MQTT](https://github.com/hareeshmu/kpr-miraie-mqtt)**
Home Assistant integration (install both).
...
2. URL: `https://github.com/hareeshmu/kpr-miraie-card`
3. Category: **Dashboard** (or **Plugin** in older HACS)
4. Click **ADD** → find **KPR MirAIe Card** → **DOWNLOAD**
5. HACS drops the file into `/config/www/community/kpr-miraie-card/` and
   registers the Lovelace resource automatically.
6. Hard-refresh browser (`Cmd+Shift+R`). Console should log:
   `KPR-MIRAIE-CARD v1.3.0`.

### Manual

1. Download `kpr-miraie-card.js` from the
   [latest release](https://github.com/hareeshmu/kpr-miraie-card/releases/latest)
   assets.
2. Copy to your HA `/config/www/` (or any subfolder).
